...
vector < string > QueryExpand::split(string s,
...
void QueryExpand::extractTerms(map < int, map < int, set < int > > > & mQueryDoc,
  ...
  for (map < int, map < int, set < int > > > ::iterator mQueryIter = mQueryDoc.begin(); mQueryIter != mQueryDoc.end(); ++mQueryIter) {
    /* 
